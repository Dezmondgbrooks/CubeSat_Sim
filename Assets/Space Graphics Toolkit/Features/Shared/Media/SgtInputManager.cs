...
namespace SpaceGraphicsToolkit
{
	...
	[AddComponentMenu(SgtHelper.ComponentMenuPrefix + "Examples/Input Manager")]
	public class SgtInputManager : MonoBehaviour
	{
		...
		public struct Bind
		{
			...
			public float GetValue(List<Finger> fingers)
			{
				var value = 0.0f;

				value -= IsPressed(KeyNegative) == true ? KeySensitivity : 0.0f;
				value += IsPressed(KeyPositive) == true ? KeySensitivity : 0.0f;

				value -= IsPressed(KeyNegativeAlt) == true ? KeySensitivity : 0.0f;
				value += IsPressed(KeyPositiveAlt) == true ? KeySensitivity : 0.0f;

				if (FingerCount > 0 && fingers.Count == FingerCount)
				{
					switch (FingerGesture)
					{
						case FingerGesture.HorizontalDrag: value += GetAverageDeltaScaled(fingers).x * FingerSensitivity; break;
						case FingerGesture.VerticalDrag: value += GetAverageDeltaScaled(fingers).y * FingerSensitivity; break;
						case FingerGesture.Twist: value += GetAverageTwistRadians(fingers) * FingerSensitivity; break;
					}
				}
		...
		{
			if (Application.isPlaying == true && FindObjectOfType<SgtInputManager>() == null)
			{
				new GameObject("SgtInputManager").AddComponent<SgtInputManager>();
			}
		}
