	...
	public class SgtShaderBundle : ScriptableObject
	{
		...
		private static void AutoSwitch()
		{
			...
			if (lastAutoSetPipe != pipe)
			{
				...
				var modified = false;
				var guids    = UnityEditor.AssetDatabase.FindAssets("t:" + typeof(SgtShaderBundle).Name);
				...
				if (modified == true)
				{
					Debug.Log("SgtShaderManager changed shaders to pipeline: " + pipe);
				}
			}
		...
		public void TrySwitchTo(Pipeline pipe)
		{
...
namespace SpaceGraphicsToolkit
{
	...
	[CustomEditor(typeof(SgtShaderBundle))]
	public class SgtShaderBundle_Editor : Editor
	{
		private SgtShaderBundle tgt;
			...
			EditorGUI.BeginDisabledGroup(true);
				foreach (var guid in guids)
				{
					...
					if (bundle != null)
					{
						...
						EditorGUILayout.ObjectField(title, bundle, typeof(SgtShaderBundle), false);
...
namespace SpaceGraphicsToolkit
{
	...
	public class SgtShaderBundleDetector : AssetPostprocessor
	{
