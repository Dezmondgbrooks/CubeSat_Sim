...
namespace SpaceGraphicsToolkit
{
	...
	public class SgtLight : SgtLinkedBehaviour<SgtLight>
	{
		...
		/// <summary>This allows you to control the intensity of the this light when used by SGT components that implement their own custom lighting (e.g. SgtAtmosphere). This should typically be in the 0..1 range.</summary>
			...
			for (var i = 0; i < tempLights.Count; i++)
			{
				...
				for (var j = SgtHelper.tempMaterials.Count - 1; j >= 0; j--)
				{
					...
					if (tempMaterial != null)
					{
						...
						tempMaterial.SetColor(properties.Color, SgtHelper.ToGamma(color));
						tempMaterial.SetColor(properties.Scatter, SgtHelper.ToGamma(color * scatterStrength));
					}
				}
...
namespace SpaceGraphicsToolkit
{
	...
	public class SgtLight_Editor : SgtEditor<SgtLight>
	{
		protected override void OnInspector()
		{
			...
			Draw("intensityInSGT", "This allows you to control the intensity of the this light when used by SGT components that implement their own custom lighting (e.g. SgtAtmosphere). This should typically be in the 0..1 range.");
